...
#if S_WIN32
  unsigned short int fat_date, fat_time;
...
{ register int msd_a;
  ...
  if      (msd_pat[0] == 0 ||
	  !*match_fn_re_ic(s,msd_pat, S_MSDOS && S_WIN32 == 0 ? true : msd_ic)
          )
    msd_attrs |= MSD_MATCHED;
  else if (not (msd_attrs & MSD_DIRY) or not (msd_props & MSD_AUTO))
    return false;
...
/*eprintf(null, "FatDate %lx FatTime %lx\n", fat_date, fat_time);*/
  msd_stat.st_mtime = ((Int)fat_date << 16) + fat_time;
#elif S_MSDOS
  msd_a =  dta[0x15] & 0x3f;
  if (msd_a & MSD_DIRY)
  { 
...
/*if (msd_a & MSD_SLINK)
  ...
  if (msd_nochdir == 0 and msd_slnest == 0 and msd_chd < msd_ix - 1 and
      (msd_a & MSD_DIRY) and msd_nlink[msd_ix] <= -1000000+1)/*stat the parent*/
...
/*eprintf(null, "DFF %d\n", msd_cc);*/
  return msd_cc;
#else
{ register char * dir = pathend == 0 ? "." : msd_relpath;
...
/*eprintf(null, "PTRY %d %d %d\n", msd_ix, msd_chd, msd_nlink[msd_ix]);*/
  ...
  /*eprintf(null, "ptry\n");*/
    ...
    if      ((msd_props & MSD_CHGD) or msd_ix == 0)
    {
#if S_WIN32
      char cwdb[300];
      fd = chdir(dir);
