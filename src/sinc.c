...
{ int cplim = lp->linelength;
  ...
  return ix;
					...
					/* < 0 : not found */
static
int Pascal strxct_mtch(const char ** t_, const char * s_, 
...

static char * Pascal find_nch(char * cp, int cplen)
...
#if AM_ME
static
     int Pascal doit_forward(meLine * basel, meLine * lp, char * cp_)
#else
static
     int Pascal doit_forward(LINE * lp, Char * cp_)
#endif	
{ int  word_ct = M_NOGO;
 register
  int ch;
      ...
      { if (paren_dpth < 2 && isword((char)ch))
        {
          if ((state & M_IN_W) == 0)
          { 
            ch = strxct_mtch(ops, cp, cplim, upper_index(ops)+1);
            if (ch >= 0)
				    { if      (ch > 9)
				        word_ct = M_NOGO;
...
static meBuffer * outbuffer;		/* buffer accumulating includes */
...
{          int    sz = strlen(str);
  ...
  return TRUE;
				 ...
				 /* result: 0 => not found, -1 => aborted, else
					    type of occurrence */
Pascal srchdeffile(char * fname, char * str, int depth)
...
#else
  extern char gs_buf[NSTRING];		/* from input.c */
  ...
  { int is_directive = false;
    ...
    { cp = skipspaces(cp+1, cplim);
      ...
      { ch = *cp;
        ...
        if (ch == 'd')
        { if (strxct_mtch(&str, cp, cplim + 1, 1) >= 0 && outbuffer == NULL)
...
#define b ((char*)&gs_buf[128])
            ...
            if (outbuffer != NULL)
				      (void)bufappline(outbuffer->b_baseline->l_fp, depth * 3, fnb);
...
#define cc ix  
  ...
  { if (cc > 0)
    ...
    { mlwrite(MWABORT, "Found type %d", cc);
      if (best_bp != NULL)
        swbuffer(frameCur->windowCur, best_bp);
    }
    else
    { /*mbwrite("[Not Found]");*/
      mlwrite(MWABORT, 
  ...
  for ( lp = (meLine*)namelist; lp != null; lp = (meLine*)namelist)
  { namelist = *(char**)lp;
    free((char*)lp);
  }
...
{ static const char * fistr = "file incls";
  ...
  outbuffer = bfind(fistr, TRUE, 0);
  if (outbuffer == NULL)
    return 0;

  if (outbuffer->b_fname == NULL)
    outbuffer->b_fname = strdup(fistr+5) ;

{ int cc = bclear(outbuffer);
  if (cc != TRUE)
	  return cc;

  cc = searchIncls(f, n);
  if (outbuffer->b_baseline->l_fp != outbuffer->b_baseline)
  { outbuffer->b_flag |= BFACTIVE;
    swbuffer(outbuffer);
  }
  else
