 ...
 */
static char Pascal nextch(Lpos_t * lpos, int dir)
{
	int c = '\n';
  LINE * curline = lpos->curline;
  int	curoff = lpos->curoff;
...
{	BITMAP  * p_bmap = cclarray[to];
	...
	mcptr->mc_type = CCL | ((pat[patix+1] != MC_NCCL) ? 0 : NOT_PAT);
	if (mcptr->mc_type & NOT_PAT)
		++patix;
...
{   register int cc;
    ...
    cc = strlen(concat(&tpat[0], prompt, " [", null));
							/* add old pattern */
    ...
    if (lpos.curline == bl)
      return false;
	   ...
	   /* Save the current position in case we need to
	     * restore it on a match, and initialize g_Dmatchlen to
...
{
	int cc;	/* success flag on pattern inputs */
...
/*			     "' with '" */
	...
	while (--n >= 0 && scanner(FORWARD, FALSE))
...
{ Lpos_t sv = *(Lpos_t*)&curwp->w_dotp;	/* original line pointer */
  ...
  if (fin - offs >= sizeof(buff))
		fin = offs + sizeof(buff) - 2;
