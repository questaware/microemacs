...
char *Pascal namval();
BUFFER * USE_FAST_CALL Pascal prevele(int, BUFFER*);
char *Pascal timeset();
...
int	USE_FAST_CALL set_buf_tab(BUFFER*bp, int tabz);
...
int Pascal USE_FAST_CALL lchange(register int);
...
int USE_FAST_CALL linsert(int, char);
...
int Pascal USE_FAST_CALL nextch(Lpos_t * lpos, int dir);
